...
export const config: Options.Testrunner = {
  ...
  before: async function (capabilities, specs) {
    ...
    let responseCreateUserApi = await usersAPI.post(
      user.username,
      user.email,
      user.password
    );
    // We check if the user is created with greater than 500 because the API is returning 409 when the user is already created
