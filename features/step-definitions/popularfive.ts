...
Then(
  /^I should see the posts older than five day with the date in red$/,
  async () => {
    //We use the points to assurence the post created more than 5 days ago is the first
    await expect(await PostPage.dateCreateFirstPost).toHaveStyle({
...
Then(
  /^I should see the posts created a five days with the date in the default color$/,
  async () => {
    //We use the points to assurence the post created five days ago is the second
    await expect(await PostPage.dateCreateSecondPost).toHaveStyle({
      color: "rgba(0,0,0,1)",
    });
  }
);
...
Then(
  /^I should see the posts created today with the date in the default color$/,
  async () => {
    //We use the points to assurence the post created today is the third
    await expect(await PostPage.dateCreateThirdPost).toHaveStyle({
      color: "rgba(0,0,0,1)",
    });
  }
);
